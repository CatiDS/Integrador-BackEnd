...
//             const consulta = `insert into cuenta_mesa (reserva , mesa) VALUES (?, ?)`;

//             const result = await db.execute(consulta, params);
//             return { message: 'Exito al crear cuenta', detalle: result };
//         } catch (error) {
//             if (error.code === 'ER_NO_DEFAULT_FOR_FIELD') {
//                 throw new Error('No se pudo realizar la cuenta, existen datos requeridos que no han sido cargados.' + error.message);
//             } else
//                 if (error.code === 'ER_TRUNCATED_WRONG_VALUE_FOR_FIELD') {
//                     throw new Error(`No se pudo realizar la cuenta, debe ingresar el id_reserva de la reserva correspondiente. ` + error.message);
//                 } else if (error.code === 'ER_NO_REFERENCED_ROW_2') {
//                     throw new Error(`No se pudo realizar la cuenta, el id_reserva:  ${datos.reserva} no se encuentra en la base de datos: ` + error.message);
//                 } else {
//                     throw new Error('No se pudo realizar la cuenta debido a: ' + error);
//                 }
//         }
//     },
